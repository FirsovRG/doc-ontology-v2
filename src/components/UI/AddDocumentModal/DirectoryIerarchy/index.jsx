...
const DirectoryIerarchy = React.memo(({ selectedDir, onDirSelect }) => {
      ...
      <li key={doc.name} className={styles.documentsDirectory}>
        <div className={styles.directoryTitle}>
          ...
          >
            ...
            doc.children.some((item) => item.type === "directory") ? (
          ...
          <div
            className={cx(
              styles.itemTitle,
              selectedDir === doc.path && styles.selectedDir
            )}
            onClick={() => onDirSelect(doc.path)}
          >
            {doc.name}
          </div>
        </div>
  ...
  return (
    <div className={styles.treeIerarchy}>
      <ul>
        {[{ type: "directory", name: "/", path: "", children: ierarchy }].map(
          renderDirTree
        )}
      </ul>
    </div>
